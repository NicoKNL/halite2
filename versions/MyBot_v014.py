...
class CommandCenter(object):
    ...
    def determine_tasks(self):
        ...
        for unit in self.units:
            ...
            if ((len(self.owned_ships) > 3) and (self.turn < 50) and (self.early_hunters < 2)) or current_task == "early_hunter":
                # logging.info(f"------------assigning hunter: {ship.id}")
                if current_task != "early_hunter":
            ...
            if (len(self.empty_planets) > 0) and (current_task not in ["early_hunter", "defensive_swarm"]):
                unit.set_task("collonize_planet")
            ...
            if len(self.enemy_planets) > 0:
                ...
                planets_by_distance = list(sort_entities_by_distance(ship, self.enemy_planets).items())
                enemy_planet = planets_by_distance[0][0]
                unit.set_target(enemy_planet)
                unit.set_task("offensive_swarm")
                self.update_targeted_planets(unit, enemy_planet)
                continue
    ...
    def execute_tasks(self):
        ...
        for unit in self.units:
            ...
            elif task == "early_hunter":
                command = self.early_hunter(unit)
    ...
    def avoid_danger(self, unit):
        ...
        return ship.navigate(target_position,
                             ...
                             ignore_ships=False,
                             max_corrections=10, angular_step=9)
...
game = hlt.Game("CommandCenterV02")
